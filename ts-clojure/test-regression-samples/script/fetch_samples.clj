...
(defn -main
  [& _args]
  (when (not (fs/exists? cnf/tr-repos-root))
    (fs/create-dir cnf/tr-repos-root))
  (with-open [rdr (clojure.java.io/reader cnf/tr-repos-list-path)]
    (doseq [line (line-seq rdr)]
      (let [[_ url user name ref]
            ;; skip things that don't start with https://
            (re-matches #"^(https://.*/([^/\t]+)/([^/\t]+))\t([^ ]+)$" line)
            user-dir (str cnf/tr-repos-root "/" user)
